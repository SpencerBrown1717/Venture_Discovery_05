...
function cardHtml(c) {
  ...
  const memoBtn = c.memo ? `<button class="memo-btn" data-memo="${escapeHtml(c.id)}">📄 Memo</button>` : "";
  return `
    <div class="card">
      <div class="card-top">
        <div>
...
function openMemo(id) {
  ...
  $("drawer").innerHTML = `
    ...
    <div class="memo-sub">${escapeHtml(m.one_liner || "")} <span class="badge-gen">${escapeHtml(m.generated_by || "heuristic")}${m.website_analyzed ? " · site analyzed" : ""}</span></div>
    <div class="memo-grid">
      <div class="memo-kv"><div class="k">Market category</div><div class="v">${escapeHtml(m.market_category || "—")}</div></div>
    ...
    <div class="memo-sec"><h4>Investment thesis</h4><p>${escapeHtml(m.thesis || "")}</p></div>
    <div class="memo-sec"><h4>Reasoning</h4><p>${escapeHtml(m.reasoning || "")}</p></div>
    ${m.market_note ? `<div class="memo-sec"><h4>Market read</h4><p>${escapeHtml(m.market_note)}</p></div>` : ""}
    <div class="memo-sec"><h4>Key risks</h4><ul>${risks}</ul></div>
